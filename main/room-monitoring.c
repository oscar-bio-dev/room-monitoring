...
static void sensor_orchestration_task(void *pvParameters) {
    ...
    bool bme_initialized = (bme688_bsec_init(bme688_dev) == 0);
    ...
    while (1) {
        ...
        if (state == PM_STATE_WAKE_A) {
            ...
            if (bme_initialized) {
                bme688_bsec_read_iaq(&rtc_iaq, &rtc_acc, &rtc_temp, &rtc_hum);
...
void app_main(void) {
    ...
    xTaskCreatePinnedToCore(sensor_orchestration_task, "Sensor_Orchestrator", 16384, (void *) bus_handle, 5, NULL, 1);
}
