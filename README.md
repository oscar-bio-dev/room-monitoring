...
- [x] **Fase 4d:** (Auditoría Técnica Bloque 4) Validación Estricta de ACK y Transaccionalidad SD. Corrección de bug de borrado de telemetría por ACK corrupto (validación post-decode Protobuf). Implementación de compactación SD crash-safe (tolerante a cortes de energía) y replay de backups rotados (`offline_bak.dat`).
- [ ] **Fase 4e:** (Integración Ecosistema) Pruebas End-to-End (E2E) en Laboratorio con el Edge-Telemetry-Gateway. Validación de la inyección asíncrona de comandos (`CMD_RUN_SELF_TEST`) y sincronización automática de Epoch en el `GatewayAck`.
- [ ] **Fase 5a:** (Auditoría Técnica Bloque 5) Endurecimiento de Seguridad para Producción. Habilitar Cifrado de NVS (`CONFIG_NVS_ENCRYPTION=y`) para proteger parámetros sensibles. Restringir Commissioning BLE limitando intentos de emparejamiento (Pairing) e integrando un botón físico de Reset (GPIO) para forzar re-aprovisionamiento seguro.
- [ ] **Fase 5b:** (Aprovisionamiento Enterprise) Validación E2E con la App Móvil (Setae Connect). Lectura óptica de Código QR y configuración Zero-Touch vía BLE (MAC e inyección del Epoch desde el smartphone del operario).
