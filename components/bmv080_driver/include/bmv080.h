...
bmv080_status_code_t bmv080_open(bmv080_handle_t *handle, const bmv080_sercom_handle_t sercom_handle,
                                 const bmv080_callback_read_t read, const bmv080_callback_write_t write,
 ...
 * @return E_BMV080_OK if successful, otherwise the return value is a BMV080 status code.
 */
bmv080_status_code_t bmv080_get_driver_version(uint16_t *major, uint16_t *minor, uint16_t *patch, char git_hash[12],
                                               int32_t *num_commits_ahead);
 ...
 * <tr> <th> Key                             <th> Type                 <th> Unit      <th> Default <th> Details <tr>
 * <td> "error_logging"                 <td> bool                 <td>           <td> false                      <td>
 * Set if error log files are created. <br> Disabled if _false_, enabled if _true_. <tr> <td> "meta_data_logging" <td>
 * bool                 <td>           <td> false                      <td> Set if meta data log files are created. <br>
 * Disabled if _false_, enabled if _true_. <tr> <td> "path"                          <td> char*                <td> <td>
 * "" (empty)                 <td> Path to directory where log files are written. <br> The maximum allowed length is 256
 * characters, which must be pre-allocated. <br> File logging is not available on embedded platforms (e.g. ARM
 * Cortex-M). <br> This is applicable to all parameters with the suffix "_logging" in its name. <tr> <td> "pm_logging"
 * <td> bool                 <td>           <td> false                      <td> Set if particulate matter output log
 * files are created. <br> Disabled if _false_, enabled if _true_. <tr> <td> "integration_time"              <td> float
 * <td> s         <td> 10                         <td> Measurement window. <br> In duty cycling mode, this measurement
 * window is also the sensor ON time. <tr> <td> "duty_cycling_period"           <td> uint16_t             <td> s <td> 30
 * <td> Duty cycling period (sum of integration time and sensor OFF / sleep time). <br> This must be greater than
 * integration time by at least 2 seconds. <tr> <td> "do_obstruction_detection"      <td> bool                 <td> <td>
 * true                       <td> Set if obstruction detection feature is enabled. <tr> <td> "do_vibration_filtering"
 * <td> bool                 <td>           <td> false                      <td> Set if vibration filter is enabled.
 * <tr> <td> "measurement_algorithm"         <td> bmv080_measurement_algorithm_t <td> <td>
 * E_BMV080_MEASUREMENT_ALGORITHM_HIGH_PRECISION (3) <td> Selection of measurement algorithm based on the <br> use case,
 * as defined by the type bmv080_measurement_algorithm_t, <br> in bmv080_defs.h. For a duty cycling measurement, this
 * parameter is <br> fixed to E_BMV080_MEASUREMENT_ALGORITHM_FAST_RESPONSE.
 * </table>
 *
 * @pre A valid _handle_ generated by _bmv080_open_ is required.
 *
 ...
 * bool                 <td>           <td> false                      <td> Set if meta data log files are created. <br>
 * Disabled if _false_, enabled if _true_. <tr> <td> "path"                          <td> char*                <td> <td>
 * "" (empty)                 <td> Path to directory where log files are written. <br> The path length must be less than
 * 256 characters. <br> Relative and absolute paths with separators ( / ) or ( \\ ) are supported. <br> ( . ) indicates
 * the working directory and ( .. ) the parent directory. <br> File logging is not available on embedded platforms (e.g.
 * ARM Cortex-M). <br> This is applicable to all parameters with the suffix "_logging" in its name. <tr> <td>
 * "pm_logging"                    <td> bool                 <td>           <td> false                      <td> Set if
 * particulate matter output log files are created. <br> Disabled if _false_, enabled if _true_. <tr> <td>
 * "integration_time"              <td> float                <td> s         <td> 10                         <td>
 * Measurement window. <br> In duty cycling mode, this measurement window is also the sensor ON time. <tr> <td>
 * "duty_cycling_period"           <td> uint16_t             <td> s         <td> 30                         <td> Duty
 * cycling period (sum of integration time and sensor OFF / sleep time). <br> This must be greater than integration time
 * by at least 2 seconds. <tr> <td> "do_obstruction_detection"      <td> bool                 <td>           <td> true
 * <td> Set if obstruction detection feature is enabled. <tr> <td> "do_vibration_filtering"        <td> bool <td> <td>
 * false                      <td> Set if vibration filter is enabled. <tr> <td> "measurement_algorithm"         <td>
 * bmv080_measurement_algorithm_t <td> <td> E_BMV080_MEASUREMENT_ALGORITHM_HIGH_PRECISION (3) <td> Selection of
 * measurement algorithm based on the <br> use case, as defined by the type bmv080_measurement_algorithm_t, <br> in
 * bmv080_defs.h. For a duty cycling measurement, this parameter is <br> fixed to
 * E_BMV080_MEASUREMENT_ALGORITHM_FAST_RESPONSE.
 * </table>
 *
 * @pre A valid _handle_ generated by _bmv080_open_ is required.
 * @pre This function must be called before _bmv080_start_continuous_measurement_ or
 * _bmv080_start_duty_cycling_measurement_ in order to apply the parameter in the configuration of particle measurement.
 *
 * @param[in] handle : Unique handle for a sensor unit.
 ...
 *
 * @pre A valid _handle_ generated by _bmv080_open_ is required.
 * @pre Optionally, duty cycling parameters (integration_time and duty_cycling_period) can be set by preceding
 * _bmv080_set_parameter_ calls.
 * @post The sensor unit stays in duty cycling mode until _bmv080_stop_measurement_ is called.
 * @post In measurement mode, _bmv080_serve_interrupt_ should be called regularly.
 *
 * @param[in] handle : Unique handle for a sensor unit.
 * @param[in] get_tick_ms : Function pointer that provides a tick value in milliseconds (based on the host system
 * clock).
 ...
 *      the sensor unit entered measurement mode via _bmv080_start_continuous_measurement_ or
 * _bmv080_start_duty_cycling_measurement_.
 * @pre The application can call this function every time the sensor or the application triggers an interrupt.
 *      This interrupt may be a type of a software timeout (e.g. at least once per second) or a hardware interrupt (e.g.
 * FIFO watermark exceeded).
 * @pre This function tolerates very frequent or random calls to a certain extent.
 *      However, not calling _bmv080_serve_interrupt_ over longer periods might impair the measurement mode since events
 * might be missed.
 ...
 *      the callback function _data_ready_ would subsequently be called 5 times to report the collected sensor output of
 * each period.
 ...
 * @pre The callback function _bmv080_callback_data_ready_t_ and the according _callback_parameter_ are provided by the
 * caller application.
 ...
 * @return E_BMV080_OK if successful, otherwise the return value is a BMV080 status code.
 */
bmv080_status_code_t bmv080_serve_interrupt(const bmv080_handle_t handle, bmv080_callback_data_ready_t data_ready,
 ...
 *      the sensor unit entered measurement mode via _bmv080_start_continuous_measurement_ or
 * _bmv080_start_duty_cycling_measurement_.
 * @pre Must be called at the end of a data acquisition cycle to ensure that the sensor unit is ready for the next
 * measurement cycle.
 *
 * @param[in] handle : Unique handle for a sensor unit.
 *
