...
typedef enum {
     ...
     *
     * 1 - IAQ baseline adaptation is disabled and wait up to 10 seconds before restarting adaptation when input is
     *reset to 0.
     *
     ...
     *
     *	@note Baseline is a mechanism for compensating the long term drifts in the enviornment and ensure the
     *consistency of IAQ and TVOC equivalent outputs across different BME sensors.
     */
...
typedef enum {
     ...
     * The self heating introduced by the heater is depending on the sensor operation mode and the sensor supply
     * voltage.
     *
     *
     ...
     * * ```BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_TEMPERATURE = ::BSEC_INPUT_TEMPERATURE -  function(sensor operation
     * mode, sensor supply voltage) - ::BSEC_INPUT_HEATSOURCE```
     *
     *
     * The self-heating in operation mode BSEC_SAMPLE_RATE_ULP is negligible.
     * The self-heating in operation mode BSEC_SAMPLE_RATE_LP is supported for 1.8V by default (no config file
     * required). If the BME6xy sensor supply voltage is 3.3V, the corresponding config file shall be used.
     */
...
typedef enum {
    ...
    BSEC_E_DOSTEPS_INVALIDINPUT = -1, /*!< Input (physical) sensor id passed to bsec_do_steps() is not in the valid
    ...
    BSEC_E_SU_WRONGDATARATE     = -10, /*!< The sample_rate of the requested output (virtual) sensor passed to
    ...
    BSEC_E_SU_MULTGASSAMPLINTVL =
        -17, /*!< The sample_rate of the requested output (virtual), which requires the gas sensor, is not equal to the
    ...
    BSEC_E_CONFIG_FEATUREMISMATCH =
        -35, /*!< Enabled features encoded in serialized_[settings/state] passed to bsec_set_[configuration/state]()
...
typedef struct {
     ...
     *
     * Timestamps must be provided as non-repeating and increasing values. They can have their 0-points at system start
     * or at a defined wall-clock time (e.g., 01-Jan-1970 00:00:00)
     */
    int64_t time_stamp;
    float
        signal; /*!< @brief Signal sample in the unit defined for the respective sensor_id @sa bsec_physical_sensor_t */
...
typedef struct {
     ...
     * box with exhaled breath) for auto-trimming  | |                            |   2   | Medium accuracy:
     * auto-trimming ongoing | |                            |   3   | High accuracy
     *
     ...
     * | |                            |   3   | Predicted regression estimate output is within the the value of the
     * label present in the training data                                                          |
     *
...
typedef struct {
    ...
    uint32_t
        process_data;            /*!< @brief Bit field describing which data is to be passed to bsec_do_steps() @sa
